...
12. local database evidence 中带有 case_id 或 article_id 的内容来自本项目数据库。回答时优先参考这些材料，并明确区分“本地数据库记录”与“一般医学知识”；数据库没有支持的结论不要伪装成数据库证据。
""".strip()
...
def build_request_policy(provider: str, mode: str) -> RequestPolicy:
    ...
    return RequestPolicy(
        ...
        pool_timeout=10.0,
        max_retries=0,
        max_output_tokens=2500 if mode_name == "initial_patient_analysis" else 2000,
