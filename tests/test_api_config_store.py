...
class ApiConfigStoreTests(unittest.TestCase):
    ...
    def test_saved_config_is_masked_but_resolves_full_key_locally(self):
        ...
        self.assertEqual(masked["api_key_masked"], "sk-l...3456")
        self.assertEqual(resolved["api_key"], "sk-local-secret-123456")

    def test_history_config_can_be_activated_without_resubmitting_key(self):
        first = self.store.save_verified({"api_key": "sk-first-111111", "provider": "openai", "model": "gpt-4.1-mini"})
        self.store.save_verified({"api_key": "sk-second-222222", "provider": "deepseek", "model": "deepseek-chat"})
        ...
        self.assertEqual(resolved["api_key"], "sk-first-111111")

    def test_explicit_key_overrides_saved_config(self):
        self.store.save_verified({"api_key": "sk-saved-111111", "provider": "openai", "model": "gpt-4.1-mini"})

        resolved = self.store.resolve_request_config({
            "api_key": "sk-one-shot-999999",
            "provider": "custom",
            "model": "custom-model",
            "base_url": "https://example.test/v1",
        })

        self.assertEqual(resolved["api_key"], "sk-one-shot-999999")
        self.assertEqual(resolved["provider"], "custom")

    def test_clear_all_removes_current_and_history(self):
        self.store.save_verified({"api_key": "sk-secret-111111", "provider": "openai", "model": "gpt-4.1-mini"})
    ...
    def test_files_are_local_json_and_plaintext_never_appears_in_masked_payload(self):
        secret = "sk-secret-never-return-1234"
